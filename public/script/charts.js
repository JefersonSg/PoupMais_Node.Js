...
const compraLabel = document.querySelectorAll('#despesaLabel');
const vendaLabel = document.querySelectorAll('#receitaLabel');
...
emprestimosLabel.forEach((i) => {
  ...
  if (dataDoEmprestimo >= trintaDiasAtraz) {
    const ValorEmprestimo = i
      .querySelector('.valorInit')
      .innerText.replace('R$ ', '');
...
// valor dos ganhos
let ganhosTotais = ganhos.reduce(
  (acumulador, ganhosTotais) => acumulador + ganhosTotais,
  0,
);
ganhoSpan.innerText = `R$ ${ganhosTotais.toFixed(2)}`;

// valor dos gastos
let gastosTotais = gastos.reduce(
  (acumulador, gastosTotais) => acumulador + gastosTotais,
  0,
);
gastoSpan.innerText = `R$ ${(gastosTotais.toFixed(2) + '').replace('-', '')}`;

// valor dos emprestimos
let emprestimoTotal = emprestimos.reduce(
  (acumulador, emprestimoTotal) => acumulador + emprestimoTotal,
  0,
);
    ...
    datasets: [
      {
        ...
        backgroundColor: ['#F92828', '#00CF15', '#6D29D3'],
        borderWidth: 1,
      },
    ],
  },
...
function trocarDatas(dias) {
  ...
  let ganhosTotais = ganhosDiasInseridos.reduce(
    (acumulador, ganhosTotais) => acumulador + ganhosTotais,
    0,
  );
  ...
  let gastosTotais = gastosDiasInseridos.reduce(
    (acumulador, gastosTotais) => acumulador + gastosTotais,
    0,
  );
  ...
  let emprestimoTotal = emprestimosDiasInseridos.reduce(
    (acumulador, emprestimoTotal) => acumulador + emprestimoTotal,
    0,
  );
