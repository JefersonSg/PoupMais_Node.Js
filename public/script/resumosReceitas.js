const receitasLabel = document.querySelectorAll('#receitaLabel');
...
const arrayResumoReceitas = [];

receitasLabel.forEach((label) => {
  const categ = label.querySelector('#categoria').innerText;

  if (!arrayResumoReceitas.includes(categ)) {
    arrayResumoReceitas.push(categ);
  }
});
...
function atualizaH3(dias) {
  ...
  receitasLabel.forEach((receita) => {
    ...
    let valoresTotaisAnteriores = valoresTotais.reduce(
      (acomulador, valorAtual) => +acomulador + valorAtual,
      0,
    );

    resumoReceita.innerText = `R$ ${valoresTotaisAnteriores.toLocaleString(
      'pt-BR',
    )}`;
  });
}
...
function valoresComparados(dias) {
  ...
  const porcentagemComparada = graficoResumoReceita.querySelector(
    '.porcentagemComparada',
  );
  const diasSpan = graficoResumoReceita.querySelector('.diasComparados');
  const diferencaComparada = graficoResumoReceita.querySelector(
    '.diferencaComparada',
  );
  const porcentagem = +(
    (valoresAtuaisSomados / valoresAnterioresSomados) *
    100
  ).toFixed(0);
  ...
  if (valoresAnterioresSomados < valoresAtuaisSomados) {
    ...
    const diferenca = valoresAtuaisSomados - valoresAnterioresSomados;
    if (porcentagem !== Infinity) {
      if (porcentagem == 0 && valoresAnterioresSomados !== 0) {
      ...
      porcentagemComparada.innerText = `deu erro mais que foi no menor`;
    }
    ...
    porcentagemComparada.innerText = `0% a mais que nos`;
    diasSpan.innerText = `${dias} dias anteriores`;
...
function setarValores() {
  ...
  let novoArrayValoresTop4 = arrayValoresColetados.map((item) => item);
  if (arrayValoresColetados > 4) {
  ...
  divs.forEach((div, n) => {
    ...
    let valoresSomados = valoresTotais.reduce(
      (acomulador, valoresSomados) => +acomulador + valoresSomados,
      0,
    );
    ...
    if (novoArrayValoresTop4[n]) {
      porcentagem = (novoArrayValoresTop4[n][0] / valoresSomados) * 100;
      nomeDaCategoria.innerText = novoArrayValoresTop4[n][1];
      valorDiv.innerText = novoArrayValoresTop4[n][0].toLocaleString('pt-BR');
      graficoVerde.style.width = `${porcentagem.toFixed(0)}%`;
      porcentagemNumerica.innerText = `${porcentagem.toFixed(0)}%`;
    } else {
...
function adicionaValores() {
  ...
  let valoresSomados = valoresTotaisResumoTotal.reduce(
    (acomulador, valoresSomados) => +acomulador + valoresSomados,
    0,
  );
  ...
  divs.forEach((div, n) => {
    ...
    if (novoArrayValores[n]) {
      let porcentagem = +(
        (novoArrayValores[n][0] / valoresSomados) *
        100
      ).toFixed(0);
