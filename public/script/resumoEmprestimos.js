...
const resumoEmprestimos =
  graficoResumoEmprestimos.querySelector('.valorResumido');
const divResumoEmprestimo =
  graficoResumoEmprestimos.querySelector('.resumosDivs');
const divsEmprestimo = divResumoEmprestimo.querySelectorAll(
...
function atualizaH3Emprestimos(dias) {
  ...
  emprestimosLabels.forEach((Emprestimo) => {
    ...
    if (dataDoEmprestimo > diasInseridos) {
      const valor = Emprestimo.querySelector('#valor').innerText.replace(
        'R$ ',
        '',
      );
      valoresTotais.push(Math.abs(+valor));
    }
    let valoresTotaisAnvaloresAnteriores = valoresTotais.reduce(
      (acomulador, valorAtual) => +acomulador + valorAtual,
      0,
    );
    resumoEmprestimos.innerText = `R$ ${valoresTotaisAnvaloresAnteriores.toLocaleString(
      'pt-BR',
    )}`;
  });
}
...
function coletarValoresEmprestimos(dias) {
  emprestimosLabels.forEach((Emprestimo) => {
    ...
    if (dataDaEmprestimo > diasInseridos) {
      ...
      valoresTotaisEmprestimos.push(Math.abs(valor));
      valoresTotaisEmprestimosResumoTotal.push(Math.abs(valor));
    }
  });
}
...
function ValoresFiltradosPorDiasEmprestimos(dias) {
  emprestimosLabels.forEach((emprestimo) => {
    ...
    const nomeDoEmprestimo = emprestimo
      .querySelector('#nomeMov')
      .innerText.replace('Emprestou para ', '');
    const valor = +emprestimo
      .querySelector('#valor')
      .innerText.replace('R$ ', '');
    const dataDoEmprestimo = emprestimo.querySelector('#data').innerText;
    const valorFinal = +emprestimo
      .querySelector('.valorFinal')
      .innerText.replace('Total ', '');

    const lucro = Math.abs(valorFinal) - Math.abs(valor);

    if (dataDoEmprestimo > diasInseridos) {
      arrayValoresColetadosEmprestimos.push([
        Math.abs(valor),
        nomeDoEmprestimo,
        lucro,
      ]);
      arrayValoresColetadosEmprestimoResumoTotal.push([
        Math.abs(valor),
        nomeDoEmprestimo,
        lucro,
      ]);
...
function setarValoresEmprestimos(dias) {
  ...
  const lucros = arrayValoresColetadosEmprestimos.map(
    (valor) => valor[valor.length - 1],
  );
  const lucrosTotais = lucros.reduce(
    (acomulador, lucrosTotais) => +acomulador + +lucrosTotais,
    0,
  );

  if (lucrosTotais) {
    lucrosDiv.innerText = `Lucrou R$ ${lucrosTotais.toLocaleString(
      'pt-BR',
  ...
  const novoArrayValoresTop4 = arrayValoresColetadosEmprestimos.map(
    (item) => item,
  );
  ...
  divsEmprestimo.forEach((div, n) => {
    ...
    let valoresSomados = valoresTotaisEmprestimos.reduce(
...
function criaAsDivsEmprestimo() {
  const divPaiEmprestimo = document.querySelector(
    '#todasAsAtividadesEmprestimo',
  );
...
function adicionaValoresATodasAsAtividadesEmprestimo() {
  ...
  const novoArrayValores = arrayValoresColetadosEmprestimoResumoTotal.map(
    (item) => item,
  );
  let valoresSomados = valoresTotaisEmprestimosResumoTotal.reduce(
    (acomulador, valoresSomados) => +acomulador + valoresSomados,
    0,
  );
...
graficoResumoEmprestimos.addEventListener('click', function (event) {
  const botoesFiltros =
    graficoResumoEmprestimos.querySelectorAll('.botoes-filtro');
