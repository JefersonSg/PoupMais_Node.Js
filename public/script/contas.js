...
function contaCompra() {
  ...
  let valorAtual = `${parcelasInit[0].value}x de R$ ${(
    valorReplaced / parcelasInit[0].value
  ).toFixed(2)}`;
  valorInit[0].value = valorAtual;
}

InputValorInit[0].addEventListener('change', () => {
  InputValorInit[0].value = (+InputValorInit[0].value.replace(
    ',',
    '.',
  )).toFixed(2);
});
...
InputValorInit[1].addEventListener('change', () => {
  InputValorInit[1].value = (+InputValorInit[1].value.replace(
    ',',
    '.',
  )).toFixed(2);
});
function contaVenda() {
  ...
  let valorAtual = `${parcelasInit[1].value}x de R$ ${(
    valorReplaced / parcelasInit[1].value
  ).toFixed(2)}`;
  valorInit[1].value = valorAtual;
}
InputValorInit[1].addEventListener('keyup', contaVenda);
parcelasInit[1].addEventListener('change', contaVenda);

// PIX
InputValorInit[2].addEventListener('change', () => {
  InputValorInit[2].value = (+InputValorInit[2].value.replace(
    ',',
    '.',
  )).toFixed(2);
});
...
function contaEmp() {
  ...
  const valorFim =
    +InputValorInit[3].value.replace(',', '.') * jurosTotal +
    +InputValorInit[3].value.replace(',', '.');
...
InputValorInit[3].addEventListener('change', () => {
  InputValorInit[3].value = (+InputValorInit[3].value.replace(
    ',',
    '.',
  )).toFixed(2);
});
...
const EmprestimoLabelEdica = document.querySelectorAll('#emprestimoLabelEdit');
EmprestimoLabelEdica.forEach((i) => {
